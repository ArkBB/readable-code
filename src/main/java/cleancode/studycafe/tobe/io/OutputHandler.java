...
public class OutputHandler {
    ...
    public void showPassOrderSummary(StudyCafePass selectedPass, Optional<StudyCafeLockerPass> lockerPass) {
        System.out.println();
        System.out.println("이용 내역");
        System.out.println("이용권: " + selectedPass.display());

        lockerPass.ifPresent(pass -> {
            System.out.println("사물함: " + pass.display());
        });
        ...
        int totalPrice = selectedPass.getPrice()
                - discountPrice
                + lockerPass.map(StudyCafeLockerPass::getPrice).orElse(0);
